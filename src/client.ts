...
import { fetchUsage, transformUsageData, AuthenticationError } from './usage/index.js';
...
export class ClaudeUsageClient {
  ...
  async getAllAccountsUsage(): Promise<AccountUsage[]> {
    const data = await this.store.load();
    return Promise.all(
      data.accounts.map(a => this._fetchAccountUsage(a.name, a.credentials))
    );
  }
  ...
  async getAccountUsage(name: string): Promise<AccountUsage> {
    ...
    const account = data.accounts.find(a => a.name === name);
    if (!account) throw new AccountNotFoundError(name);
    return this._fetchAccountUsage(account.name, account.credentials);
  }

  private async _fetchAccountUsage(name: string, credentialsJson: string): Promise<AccountUsage> {
    try {
      ...
      if (validation.isExpired) {
        ...
        if (refreshed.success && refreshed.newCredentials) {
          ...
          return { accountName: name, session: { percent: 0, resetsAt: null }, weekly: { percent: 0, resetsAt: null }, opus: null, error: 'Token expired — refresh failed' };
        }
      }
      ...
      if (!token) return { accountName: name, session: { percent: 0, resetsAt: null }, weekly: { percent: 0, resetsAt: null }, opus: null, error: 'No access token' };

      const usage = await fetchUsage(token, this.betaVersion);
      return { accountName: name, ...transformUsageData(usage) };
    } catch (err) {
      ...
      if (err instanceof AuthenticationError) {
        ...
        return { accountName: name, session: { percent: 0, resetsAt: null }, weekly: { percent: 0, resetsAt: null }, opus: null, error: 'Authentication failed' };
      }
      return { accountName: name, session: { percent: 0, resetsAt: null }, weekly: { percent: 0, resetsAt: null }, opus: null, error: (err as Error).message };
    }
  }
